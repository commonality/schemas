...
> <img align="bottom" alt="code" height="50" width="50" src="https://cdnjs.cloudflare.com/ajax/libs/octicons/4.4.0/svg/code.svg"> This guide is for those who want to contribute code to `schemas`. This guide describes how to set up your development environment so that you can build and test `schemas`.

## 1. Recommended skills

In order to work with `schemas` as a developer, we recommend you:

* **Know JavaScript**, since `schemas` is written in JavaScript.
* Are **familiar with Node.js**, since `schemas` runs on it.
...
Before you can build and test `schemas`, you must install and configure the following products on your development machine:

1.  [Git](http://git-scm.com) and/or the **GitHub app** (for [MacOS](http://mac.github.com) or [Windows](http://windows.github.com))

    _Why:_

    > `schemas` is hosted on GitHub and uses Git for source control. In order to obtain the source code, you must first install Git on your system. Instructions for installing and setting up Git can be found at <https://help.github.com/articles/set-up-git>.

1.  [Node.js](http://nodejs.org), (version specified in the engines field of [`package.json`](../package.json)) 

    _Why:_

    > `schemas` uses Node.js modules to generate tables of contents, version, and publish documentation.

1.  **NPM** (which installs with Node.js) or [Yarn](https://yarnpkg.com)

    _Why:_

    > NPM and Yarn install and update `schemas's` third-party dependencies.

## 3. Getting the source code

Fork and clone the `schemas` repository:

1.  [**Sign in**](https://github.com/login) to your GitHub account or [sign up for a (free) GitHub account](https://github.com/join).
2.  [**Fork**](http://help.github.com/forking) the [main `schemas` repository](https://github.com/commonality/`schemas`) (aka, "`origin`").
3.  **Clone your fork** of the `schemas` repository and define an `upstream` remote pointing back to the `schemas` repository that you forked in the first place.

    > ```shell
    > # Clone your GitHub repository:
    > git clone git@github.com:<commonality>/schemas.git
    >
    ...
    > # to your repository:
    > git remote add upstream https://github.com/commonality/schemas.git
    > ```

## 4. Installing dependencies

Next, install the JavaScript modules needed to build and test `schemas`:
...
<details><summary><strong>View all <code>schemas</code> dependencies.</strong></summary>

<h3>Production dependencies</h3>

`schemas` requires the following dependencies to operate.
...
`schemas` uses the the following dependencies to build, test, or deploy:
...
![verified][octicon-verified] `schemas` uses
...
>
> `commonality/schemas` uses [Travis CI][travis-ci-url] for continuous integration and delivery.
...
[travis-ci-url]: https://travis-ci.org/commonality/schemas
[eslint-logo]: https://raw.githubusercontent.com/commonality/schemas/master/.github/assets/img/logo-eslint.png
[eslint-url]: https://eslint.org/docs/user-guide/getting-started
[jest-logo]: https://raw.githubusercontent.com/commonality/schemas/master/.github/assets/img/logo-jest.png
